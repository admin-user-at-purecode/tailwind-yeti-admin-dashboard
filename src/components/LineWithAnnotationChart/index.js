...
const LineWithAnnotationChart = (props) => {
  ...
  const lineWithAnnotationPlugin = {
    afterInit: (chart) => {
      const value = chart.data.datasets[0].data[0].toLocaleString();
  ...
  return (
    <>
      ...
      <h4 className="chart-value text-2xl mt-2">${value}</h4>
      <small className="chart-label uppercase">{label}</small>
      <div className="mt-5">
        <Chart
          type={
            withShadow ? "lineWithAnnotationAndShadow" : "lineWithAnnotation"
          }
          options={options}
          data={data}
